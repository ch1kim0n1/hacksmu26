...
export default function ComparePage() {
  ...
  return (
    ...
    >
        ...
        >
          <h1 className="text-2xl font-bold text-ev-charcoal">Cross-Species Comparison</h1>
        ...
        <motion.section
          className="mb-8 grid gap-4 rounded-2xl glass border border-ev-sand/30 p-5 md:grid-cols-[1fr_1fr_auto]"
          initial={{ opacity: 0, y: -8 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <label className="block">
            ...
            <select value={callId} onChange={(event) => setCallId(event.target.value)} className="w-full rounded-xl glass border border-ev-sand/40 px-3 py-2 text-sm focus:border-accent-savanna/50 focus:outline-none">
              {calls.map((call) => (
          ...
          <label className="block">
            ...
            <select value={referenceId} onChange={(event) => setReferenceId(event.target.value)} className="w-full rounded-xl glass border border-ev-sand/40 px-3 py-2 text-sm focus:border-accent-savanna/50 focus:outline-none">
          ...
          <button onClick={runComparison} className="self-end rounded-xl bg-gradient-to-r from-accent-savanna to-accent-savanna/90 px-5 py-2 text-sm font-semibold text-ev-ivory hover:from-accent-savanna/95 hover:to-accent-savanna/85 transition-all">
            Compare
          </button>
        </motion.section>

        {error && <div className="mb-6 rounded-xl border border-danger/15 bg-danger/5 p-4 text-sm text-danger">{error}</div>}
        ...
        {comparison && (
          ...
          >
            <section className="rounded-2xl glass border border-ev-sand/30 p-5">
              ...
              <SpectrogramImage
                ...
                height={900}
                unoptimized
                className="h-auto w-full rounded-xl border border-ev-sand/30"
                fallbackText="Comparison visualization unavailable"
              />
              <p className="mt-4 rounded-xl glass p-4 text-sm leading-relaxed text-ev-elephant">
                {String(comparison.comparison.insight)}
              </p>
            </section>
            <aside className="rounded-2xl glass border border-ev-sand/30 p-5">
              ...
              {[
                ...
                <div key={String(label)} className="mb-3 flex justify-between border-b border-ev-sand/30 pb-2 text-sm">
