...
def sync_processed_results(
    store: RecordingStore,
    processed_dir: str | Path,
    spectrogram_dir: str | Path,
    cache_dir: str | Path,
    call_database: Any | None = None,
) -> int:
    """Scan disk for processed outputs and backfill the recording store.

    Returns the number of recordings that were synced.
    """
    ...
    # Find all recording IDs that have a cleaned WAV on disk
    cleaned_files = sorted(processed_path.glob("*_cleaned.wav"))
    for cleaned_wav in cleaned_files:
        recording_id = cleaned_wav.name.replace("_cleaned.wav", "")
        recording = store.get(recording_id)
        if recording is None:
            continue
        # Skip if already complete with quality data populated
        existing_result = recording.get("result") or {}
        if (
            recording.get("status") == "complete"
            and existing_result.get("quality")
            and existing_result.get("calls") is not None
        ):
            continue

        # --- Build result from disk artifacts ---
        result: dict[str, Any] = {
            "recording_id": recording_id,
            "status": "complete",
            "output_audio_path": str(cleaned_wav),
            "stages_completed": ["ingestion", "spectrogram", "classification", "denoising", "feature_extraction", "quality_check", "complete"],
        }

        # Spectrograms
        before_spec = spectrogram_path / f"{recording_id}_before_spectrogram.png"
        after_spec = spectrogram_path / f"{recording_id}_after_spectrogram.png"
        comparison_spec = processed_path / f"{recording_id}_comparison.png"
        ...
        if comparison_spec.exists():
            result["comparison_spectrogram_path"] = str(comparison_spec)

        # Quality metrics from cache (glob for hash suffix)
        quality_files = sorted(cache_path.glob(f"{recording_id}_quality_metrics_*.json"))
        quality: dict[str, Any] = {
            "snr_before_db": 0.0,
            "snr_after_db": 0.0,
            "snr_improvement_db": 0.0,
            "spectral_distortion": 0.0,
            "energy_preservation": 0.5,
            "quality_score": 50.0,
            "quality_rating": "unknown",
        }
        if quality_files:
            try:
                loaded = json.loads(quality_files[-1].read_text(encoding="utf-8"))
                quality.update(loaded)
            except Exception:
                logger.warning("Failed to read quality metrics for %s", recording_id)
        result["quality"] = quality

        # Call markers
        markers_file = processed_path / f"{recording_id}_markers.json"
        calls: list[dict[str, Any]] = []
        if markers_file.exists():
            try:
                calls = json.loads(markers_file.read_text(encoding="utf-8"))
        ...
        # Processing time (estimate from quality if available)
        result["processing_time_s"] = quality.get("processing_time_s", 5.0)
        ...
        # Feed calls into the CallDatabase
        if call_database is not None and calls:
            try:
                call_database.upsert_processed_calls(
                    recording_id=recording_id,
                    calls=calls,
                    metadata={
                ...
                logger.warning("Failed to upsert calls for %s", recording_id)

        synced += 1
        logger.info(
            "Synced processed recording %s (%d calls, quality=%s)",
            recording_id,
            len(calls),
            quality.get("quality_rating", "unknown"),
        )

    if synced:
        logger.info("Synced %d processed recordings from disk", synced)
    return synced
